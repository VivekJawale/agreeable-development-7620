...
export default function AddAddress({check, onClick}) {
    const dispatch= useDispatch()
  return (
      
        
              <Box border= {check ? "2px solid rgb(0, 181, 181)": "2px solid gray"} bg= {check ? "rgb(237, 245, 245)": " " }>
                  <Box>
            ...
            <HStack >
                      <HStack>
                          <Box className={ Styles.select} border= {check ? "2px solid rgb(0, 181, 181)": "2px solid gray"}>
                              <VscCircleFilled size={40} fill={check ? "rgb(0,181,181)" : "gray"} onClick={onClick} />
                          </Box>
                <Text as="p">Home</Text>
              </HStack>
              <HStack color="gray">
